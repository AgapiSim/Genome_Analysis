...
#Note: To set the genome size, the NCBI was used to see if there is available info in the genome of Enterococcus faecium.
...
#Pilon uses Illumina short reads to correct base errors, samll indels and missassemblies.
...
quast.py $pacbio_path/pac_pilon_assembly.fasta -r $gen_data_path/GCF_009734005.1_ASM973400v2_genomic.fna.gz -g $gen_data_path/GCF_009734005.1_ASM973400v2_genomic.gff.gz -o $pacbio_path/pac_pilon_quast_eval
#Spades assembly
quast.py $spades_path/contigs.fasta -r $gen_data_path/GCF_009734005.1_ASM973400v2_genomic.fna.gz -g $gen_data_path/GCF_009734005.1_ASM973400v2_genomic.gff.gz -o $spades_path/spades_quast_eval

#Checking if the assemblies contain well-known reserved genes with BUSCO
source $AUGUSTUS_CONFIG_COPY                                                                                                                                                                
busco -i $spades_path/assembly.fasta -m genome -l $BUSCO_LINEAGE_SETS/enterobacterales_odb10 -o ../results/spades_assembly/spades_busco_eval -f                                      
busco -i $pacbio_path/pac_pilon_assembly.fasta -m genome -l $BUSCO_LINEAGE_SETS/enterobacterales_odb10 -o ../results/pac_assembly/pac_pilon_busco_eval 

#Relative path is adviced as when full path used BUSCO creates locally a folder with all the given path folder names
#Busco hierarchical database from which i can select a specific lineage and it contains all the conserved genes of the lineage and checks if those are found in my assembly. Basically, assess how not bad my assembly is based on the most conserved genes.
